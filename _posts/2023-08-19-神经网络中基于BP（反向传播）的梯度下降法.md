...
- **梯度下降法：** 指根据反向传播的计算结果将神经元节点间的权重**按照误差函数梯度减小的方向更新**的学习方式。
...
由于这个神经网络只有两层，所以这时就获得了该神经网络对于此次输入的一个输出值，前向传播也就完成了。接下来我们计算这个输出值与正确值的损失函数$E$。为了之后反向传播的推导式简洁，这里使用如下方式去计算:
...
反向传播时，我们应当考察该损失函数关于每一个权重$\omega_{i}$的偏导:
...
\begin{align}
    ...
    &= 
    \frac{\partial{E}}{\partial{u}} \frac{\partial{u}}{\partial{\omega_i}} \\[3mm]
    % 5
    &= \frac{\partial}{\partial{u}} \left[\frac{1}{2} (\hat{y} - y) ^ 2\right] \frac{\partial{u}}{\partial{\omega_i}} \qquad \cdots (3)\\[3mm]
    % 6
    &= \frac{\partial}{\partial{u}} \left[\frac{1}{2} (f(u) - y) ^ 2\right] \frac{\partial}{\partial\omega_i} \left(\sum_{i=0}^n x_i\omega_i\right) \qquad \cdots (2), (1) \\[3mm]
    % 7
    &= (f(u) - y)f'(u)x_i \\[3mm]
    % 8
    &= (\hat{y} - y)f'(u)x_i
...
\begin{align}
    % 9
    \omega_i^{new} 
    &= \omega_i^{old} - \eta\frac{\partial{E}}{\partial{\omega_i}} \\[3mm]
    % 10
    &= \omega_i^{old} - \eta(\hat{y} - y)f'(u)x_i
\end{align}
$$
...
\begin{equation}
    % 12
    u_j = \sum_{i=1}^m x_i\omega_{ji} \qquad (1 \leq j \leq n)
\end{equation}
$$
...
\begin{equation}
    % 14
    E = \sum_{j=1}^n\frac{1}{2}(\hat{y_j} - y_j)^2 = \sum_{j=1}^n\frac{1}{2}(f(u_j) - y_j)^2
\end{equation}
$$
...
\begin{align}
    ...
    &= \frac{\partial{E}}{\partial{u_j}} \cdot \frac{\partial{u_j}}{\partial{\omega_{ji}}} \\[3mm]
    % 16
    &= (f(u_j) - y_j)f'(u_j) \cdot x_i \\[3mm]
    % 17
    &= (\hat{y_j} - y_j)f'(u_j)x_i
\end{align}
$$
...
\begin{equation}
    % 18
    \delta_j = \frac{\partial{E}}{\partial{u_j}} = (\hat{y_j} - y_j)f'(u_j)
\end{equation}
$$
...
\begin{equation}
    % 19
    \frac{\partial{E}}{\partial{\omega_{ji}}} = \delta_j x_i
\end{equation}
$$
...
\begin{equation}
    % 20
    \omega_{ji}^{new} = \omega_{ji}^{old} - \eta \delta_j x_i
\end{equation}
$$
...
\begin{equation}
    % 22
    \boldsymbol{W_{new}} = \boldsymbol{W_{old}} - \eta\boldsymbol{\delta_j}\boldsymbol{x}^T
\end{equation}
$$
...
\begin{equation} 
    % 23
    \boldsymbol{\delta_j} = (\boldsymbol{\hat{y} - y})\odot f'(\boldsymbol{u})
\end{equation}
$$
...
### 相关计算量定义
这个神经网络的输入层、隐藏层和输出层分别具有$m, n, l$个节点，对于每一层的一个任意的节点，我们指定它的编号分别为$i, j, k$。前向传播时，从输入层到隐藏层的线性结合为$u_j^h$，从隐藏层顺到输出层的线性结合为$u_k^o$。与此对应的隐藏层和输出层的激活函数分别为$f(x), g(x)$。**权重以及其构成的权值矩阵**根据层与层之间的关系决定，三层的情况下层与层之间的关系分别为**输入层**$\leftrightarrow$**隐藏层**与**隐藏层**$\leftrightarrow$**输出层**。依照上一章第二节的[权重下标定义](#输出层为多个神经元)，可以得到输入层与隐藏层之间的权值矩阵中一个任意的权重为$\omega_{ji}$，隐藏层与输出层之间的为$\omega_{kj}$。以上定义完成后，我们便可以进行下一步的数学推导了。
...
\begin{equation}
    % 24
    u_j^h = \sum_{i=1}^m x_i\omega_{ji} \quad, \quad y_j = f(u_j^h)
\end{equation}
$$
...
\begin{equation}
    % 25
    u_k^o = \sum_{j=1}^n y_j\omega_{kj} \quad, \quad \hat{y}_k = g(u_k^o)
\end{equation}
$$
...
\begin{equation}
    % 26
    E = \frac{1}{2}\sum_{k=1}^l(\hat{y}_k - z_k)^2 = \frac{1}{2}\sum_{k=1}^l(g(u_k^o) - z_k)^2
\end{equation}
$$
...
\begin{align}
    ...
    &= \frac{\partial{E}}{\partial{u_k^o}}\frac{\partial{u_k^o}}{\partial{\omega_{kj}}} \\[3mm]
    % 28
    &= (g(u_k^o) - z_k)g'(u_k^o) \cdot y_j \\[3mm]
    % 29
    &= (\hat{y}_k - z_k)g'(u_k^o)y_j
\end{align}
$$
...
\begin{equation}
    % 30
    \delta_k^o = (\hat{y}_k - z_k)g'(u_k^o)
\end{equation}
$$
...
\begin{equation}
    % 31
    \omega_{kj}^{new} = \omega_{kj}^{old} - \eta \delta_k^o y_j
\end{equation}
$$
...
\begin{align}
    ...
    &= 
    \boldsymbol{ {\underset{H \to O}{W} }_{old}} - \eta\boldsymbol{\delta_k^o}\boldsymbol{y}^T \\[5mm]
    % 34
    \boldsymbol{\delta_k^o} 
    &= 
    (\boldsymbol{\hat{y}} - \boldsymbol{z}) \odot g'(\boldsymbol{u^o})
\end{align}
$$
...
\begin{align}
    ...
    \frac{\partial{E}}{\partial{\omega_{ji}}} 
    &= 
    \frac{\partial{E}}{\partial{y_j}}\frac{\partial{y_j}}{\partial{u_j^h}}\frac{\partial{u_j^h}}{\partial{\omega_{ji}}} \\[5mm]
    % 36
    &= 
    \frac{\partial}{\partial{y_j}}\left[\frac{1}{2}\sum_{k=1}^l(g(u_k^o) - z_k)^2\right] \cdot f'(u_j^h) \cdot \frac{\partial}{\partial{\omega_{ji}}}\left(\sum_{i=1}^m x_i\omega_{ji}\right) \qquad \cdots (26), (24) \\[5mm]
    % 37
    &= 
    \frac{1}{2}\sum_{k=1}^l \left[\frac{\partial}{\partial{y_j}}(g(u_k^o) - z_k)^2\right] f'(u_j^h)x_i \\[5mm]
    % 38
    &= 
    \frac{1}{2}\sum_{k=1}^l \left[\frac{\partial}{\partial{u_k^o}}(g(u_k^o) - z_k)^2 \cdot \frac{\partial{u_k^o}}{\partial{y_j}}\right] f'(u_j^h)x_i \\[5mm]
    % 39
    &= 
    \sum_{k=1}^l \left[(g(u_k^o) - z_k)g'(u_k^o) \cdot \frac{\partial}{\partial{y_j}} \left(\sum_{j=1}^n y_j\omega_{kj}\right) \right] f'(u_j^h)x_i \qquad \cdots (25) \\[5mm]
    % 40
    &= 
    \sum_{k=1}^l \Big[{\color{red}{(\hat{y}_k - z_k)g'(u_k^o)}}\omega_{kj}\Big] f'(u_j^h)x_i \qquad \cdots (25) \\[5mm]
    % 41
    &= 
    \sum_{k=1}^l \Big({\color{red}{\delta_k^o}}\omega_{kj}\Big) f'(u_j^h)x_i
\end{align}
$$
...
\begin{align}
    % 42
    \delta_j^h = \sum_{k=1}^l & \Big({\delta_k^o}\omega_{kj}\Big) f'(u_j^h) \\[5mm]
    % 43
    \frac{\partial{E}}{\partial{\omega_{ji}}} &= \delta_j^h x_i 
\end{align}
$$
...
\begin{equation}
    % 44
    \omega_{ji}^{new} = \omega_{ji}^{old} - \eta \delta_j^h x_i
\end{equation}
$$
...
\begin{align}
    % 50 
    & \boldsymbol{ {\underset{H \to O}{W} }_{new}} = \boldsymbol{ {\underset{H \to O}{W} }_{old}} - \eta\boldsymbol{\delta_k^o}\boldsymbol{y}^T \\[5mm]
    % 51
    & \boldsymbol{ {\underset{I \to H}{W} }_{new}} = \boldsymbol{ {\underset{I \to H}{W} }_{old}} - \eta \boldsymbol{\delta_j^h}\boldsymbol{x}^T \\[5mm]
    % 52
    & \boldsymbol{\delta_k^o} = (\boldsymbol{\hat{y}} - \boldsymbol{z}) \odot g'(\boldsymbol{u^o}) \\[5mm]
    % 53
    & \boldsymbol{\delta_j^h} = \boldsymbol{ {\underset{H \to O}{W^T} }_{old}} \boldsymbol{\delta_k^o} \odot f'\boldsymbol{(u^h)}
\end{align}
$$
...
\begin{align}
    ...
    \boldsymbol{W_{new}^{(l)}} 
    &= 
    \boldsymbol{W_{old}^{(l)}} - \eta {\color{red}{\boldsymbol{\delta^{(l)}}}} {\boldsymbol{y^{(l-1)}}}^T \\[3mm]
    % 55
    {\color{red}{\boldsymbol{\delta^{(l)}}}} 
    &=
    {\boldsymbol{W_{old}^{(l+1)}}}^T \boldsymbol{\delta^{(l+1)}} \odot f'^{(l)}(\boldsymbol{u^{(l)}})
\end{align}
$$
...
\begin{align}
    % 56
    \boldsymbol{u^{(l)}} &= \boldsymbol{W^{(l)} y^{(l-1)}} \\[3mm]
    % 57
...
### 反向传播递推公式证明
对于整个神经网络的最后一个权值矩阵$\boldsymbol{W^{(L)}}$，根据[三层神经网络](#50-53)章节中的推到结果可以得出:
...
\begin{align}
    ...
    \boldsymbol{\delta^{(L)}} 
    &= 
    (\boldsymbol{y^{(L)}} - \boldsymbol{y})f'^{(L)}(\boldsymbol{u^{(L)}}) \\[3mm]
    %60
    \boldsymbol{\delta^{(L-1)}}
    &= 
    {\boldsymbol{W^{(L)}}}^T \boldsymbol{\delta^{(L)}} \odot f'^{(L-1)}(\boldsymbol{u^{(L-1)}})
\end{align}
$$
...
%61
\begin{equation}
    \boldsymbol{(i) \; k=1}, \qquad \boldsymbol{\delta^{(L-k)}} = \boldsymbol{\delta^{(L-1)}} = {\boldsymbol{W^{[(L+1)-1]}}}^T \boldsymbol{\delta^{[(L+1)-1]}} \odot f'^{(L-1)}(\boldsymbol{u^{(L-1)}})
\end{equation}
$$
...
\begin{align}
    ...
    &= 
    \boldsymbol{\delta^{[(L+1)-{\color{red}{m}}]}} \\[3mm]
    %63
    &= {\boldsymbol{W^{[(L+1)-{\color{red}{m}}]}}}^T \boldsymbol{\delta^{[(L+1)-{\color{red}{m}}]}} \odot f'^{(L-{\color{red}{m}})}(\boldsymbol{u^{(L-{\color{red}{m}})}})
\end{align}
$$
<div id="6869"></div>
...
\begin{align}
    ...
    &=
    \boldsymbol{\delta^{[L-({\color{red}{m+1}})]}} \\[3mm]
    %65
    &=
    \frac{\partial{E}}{\partial{\boldsymbol{u^{[L-(m+1)]}}}} \\[3mm]
    %66
    &=
    \frac{\partial{E}}{\partial{\boldsymbol{u^{(L-m)}}}} \frac{\partial{\boldsymbol{u^{(L-m)}}}}{\partial{\boldsymbol{u^{[L-(m+1)]}}}} \\[3mm]
    %67
    &=
    \frac{\partial}{\partial{\boldsymbol{u^{[L-(m+1)]}}}} \left(\boldsymbol{W^{(L-m)} y^{[L-(m+1)]}}\right) \boldsymbol{\delta^{(L-m)}} \\[3mm]
    %68
    &=
    \frac{\partial}{\partial{\boldsymbol{u^{[L-(m+1)]}}}} \left(\boldsymbol{W^{(L-m)}} f^{[L-(m+1)]}(\boldsymbol{u^{[L-(m+1)]}})\right) \boldsymbol{\delta^{(L-m)}} \\[3mm]
    %69
    &=
    {\boldsymbol{W^{(L-m)}}}^T \boldsymbol{\delta^{(L-m)}} \odot f'^{[L-(m+1)]}(\boldsymbol{u^{[L-(m+1)]}}) \\[3mm]
    %70
    &=
    {\boldsymbol{W^{[(L+1)-({\color{red}{m+1}})]}}}^T \boldsymbol{\delta^{[(L+1)-({\color{red}{m+1}})]}} \odot f'^{[L-({\color{red}{m+1}})]}(\boldsymbol{u^{[L-({\color{red}{m+1}})]}})
\end{align}
$$

于是，该递推公式成立，其中关于$(68)$到$(69)$式的求导过程会放在附录中叙述。
...
%73
\begin{equation}
    \boldsymbol{\delta^{(l)}} ={\boldsymbol{W_{old}^{(l+1)}}}^T \boldsymbol{\delta^{(l+1)}} \odot f'^{(l)}(\boldsymbol{u^{(l)}})
\end{equation}
$$
...
%74
\begin{equation}
    \boldsymbol{\delta^{(l)}} = (\boldsymbol{y^{(l)}} - \boldsymbol{y})f'^{(l)}(\boldsymbol{u^{(l)}})
\end{equation}
$$
...
%75
\begin{equation}
    \boldsymbol{W_{new}^{(l)}} = \boldsymbol{W_{old}^{(l)}} - \eta \boldsymbol{\delta^{(l)}} {\boldsymbol{y^{(l-1)}}}^T
\end{equation}
$$
